...
//    public List<SerieEntity> findLastByExerciseAndUser(@Param("exerciseId") Long exerciseId, @Param("userId") Long userId);
    ...
    @Query(value = "select s from series s where s.workout.id = (select MAX(s2.workout.id) from series s2 where s2.exercise.id = :exerciseId and s2.workout.id in (select id from workouts w where w.worker.id = :userId))")
